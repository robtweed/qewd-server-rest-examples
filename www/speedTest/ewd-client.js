    ...
    (function(application, io, customAjaxFn, url) {
      ...
      if (io) {
        ...
        if (url) {
          ...
          if (typeof qewd  !== 'undefined' && qewd.io_path) {
            if (EWD.log) console.log('Setting custom socket.io path to ' + qewd.io_path);
            options.path = qewd.io_path + '/socket.io';
          }
          ...
          if (typeof qewd  !== 'undefined' && qewd.io_path) {
            if (EWD.log) console.log('Setting custom socket.io path to ' + qewd.io_path);
            socket = io({path: qewd.io_path + '/socket.io'});
          }
          else {
            socket = io.connect();
          }
